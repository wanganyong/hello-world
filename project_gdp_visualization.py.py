...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    提示：本函数可视化需要利用pygal.maps.world.World()方法
     """
    m=build_map_dict_by_name(gdpinfo, plot_countries, year)
    dict3 = m[0]
    set1 = m[1]
    set2 = m[2]
    worldmap_chart = pygal.maps.world.World()
    worldmap_chart.title = '全球GDP分布图'
    worldmap_chart.add(year,dict3)
    worldmap_chart.add("missing from World Bank", set1)
    worldmap_chart.add("no data at the year", set2)
